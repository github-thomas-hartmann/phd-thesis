 <@layout.global>
 	...
 	<@content.left>
			<div id="col1_content" class="clearfix">
				...
				<div id="xkosTabContainer" class="tabContainer">
					<ul>
						<li>
							<a class="tabElement" href="#study_tab" style="text-decoration:none;">xkos</a>
						</li>
					</ul>
...
<script>
	$jQ( function() // begin document ready
	{
		...
		$jQ( "#xkosTabContainer" ).tabs({ active: 0 });
		$jQ( "#xkosTabContainer" ).css( "margin-top", "66px");
		$jQ( "#xkosTabContainer" ).css( "margin-left", "5px");
		$jQ( "#xkosTabContainer" ).css( "margin-right", "5px");
		...
		$jQ(".graphButton").click( function(){
			...
			if( selectedNav != currentNavButton ){
				...
				$jQ.ajax( {
					type: "get",
					url:  "<@spring.url '/xkos/' />" + selectedNav
				    })
			    	.done( function( html ) {
