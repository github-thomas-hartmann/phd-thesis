...
<!-- the tabs -->
<div id="xkosEndpointTabContainer" class="tabContainer">
	<ul style="height: 40px;">	
		<li><a class="tabElement" href="#xkos-endpoint-tab1" style="text-decoration:none;padding: 10px 15px;"><span>1.</span> Constraint and SPARQL Endpoint</a></li>
		<li id="xkos-endpoint-validation-tab"><a class="tabElement" href="#xkos-endpoint-tab2" style="text-decoration:none;padding: 10px 15px;"><span>2.</span> Validation</a></li>
	</ul>
	
	<div id="xkos-endpoint-tab1" style="padding:0 !important; border:none; float:left; width:100%">
		<#include "xkos-endpoint-tab1.ftl" />
	</div>
	<div id="xkos-endpoint-tab2" style="padding:0 !important;border:none;float:left; width:100%">
		<#include "xkos-endpoint-tab2.ftl" />
	</div>
</div>
...
<script>
	$jQ( function() // begin document ready
	{
		...
		$jQ( "#xkosEndpointTabContainer" ).tabs({ 
			active: 0, 
			activate : function(event, ui){
				if(ui.newPanel.selector == "#xkos-endpoint-tab2")
					<#-- validate inputs -->
					ajaxxkosendpointValidation( "#xkos-validation-result" );
			}
     	});
     	
		<#-- submit button validation pressed -->
		$jQ( "#button_xkos-endpoint-validation" ).click( function()
		{
			<#-- validate inputs -->
			ajaxxkosendpointValidation( "#xkos-validation-result" );
		} );
	...
	function getxkosInputs( containerSelector ){
		<#-- clear -->
	...
	-->
	function ajaxxkosendpointValidation( containerResultSelector ){	
		<#-- disable validate button -->
		<!-- $jQ('#button_xkos-endpoint-validation').attr("disabled",true).addClass("buttonSubmitDissabled"); -->
		<#-- Add loading icon -->
		...
		$jQ.ajax( {
		    type: "post",
			url: "<@spring.url '/xkos/endpoint/validation' />",
			data: {
