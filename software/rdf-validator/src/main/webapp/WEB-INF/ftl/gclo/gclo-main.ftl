...
<script>
	$jQ( function() // begin document ready
	{
		...
		$jQ( "#gcloTabContainer" ).tabs({ active: 0 });
		$jQ( "#gcloTabContainer" ).css( "margin-top", "66px");
		$jQ( "#gcloTabContainer" ).css( "margin-left", "5px");
		$jQ( "#gcloTabContainer" ).css( "margin-right", "5px");
		...
		$jQ(".graphButton").click( function(){
			...
			if( selectedNav != currentNavButton ){
				...
				$jQ.ajax( {
					type: "get",
					url:  "<@spring.url '/gclo/' />" + selectedNav
				    })
			    	.done( function( html ) {
