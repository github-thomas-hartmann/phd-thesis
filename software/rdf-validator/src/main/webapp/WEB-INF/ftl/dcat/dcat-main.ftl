 <@layout.global>
 	...
 	<@content.left>
			<div id="col1_content" class="clearfix">
				...
				<div id="dcatTabContainer" class="tabContainer">
					<ul>
						<li>
							<a class="tabElement" href="#study_tab" style="text-decoration:none;">Dcat</a>
						</li>
					</ul>
...
<script>
	$jQ( function() // begin document ready
	{
		...
		$jQ( "#dcatTabContainer" ).tabs({ active: 0 });
		$jQ( "#dcatTabContainer" ).css( "margin-top", "66px");
		$jQ( "#dcatTabContainer" ).css( "margin-left", "5px");
		$jQ( "#dcatTabContainer" ).css( "margin-right", "5px");
		...
		$jQ(".graphButton").click( function(){
			...
			if( selectedNav != currentNavButton ){
				...
				$jQ.ajax( {
					type: "get",
					url:  "<@spring.url '/dcat/' />" + selectedNav
				    })
			    	.done( function( html ) {
