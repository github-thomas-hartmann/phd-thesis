 <@layout.global>
 	...
 	<@content.left>
			<div id="col1_content" class="clearfix">
				...
				<div id="shexTabContainer" class="tabContainer">
					<ul>
						<li>
							<a class="tabElement" href="#study_tab" style="text-decoration:none;">shex</a>
						</li>
					</ul>
...
<script>
	$jQ( function() // begin document ready
	{
		...
		$jQ( "#shexTabContainer" ).tabs({ active: 0 });
		$jQ( "#shexTabContainer" ).css( "margin-top", "66px");
		$jQ( "#shexTabContainer" ).css( "margin-left", "5px");
		$jQ( "#shexTabContainer" ).css( "margin-right", "5px");
		...
		$jQ(".graphButton").click( function(){
			...
			if( selectedNav != currentNavButton ){
				...
				$jQ.ajax( {
					type: "get",
					url:  "<@spring.url '/shex/' />" + selectedNav
				    })
			    	.done( function( html ) {
