...
<!-- the tabs -->
<div id="skosEndpointTabContainer" class="tabContainer">
	<ul style="height: 40px;">	
		<li><a class="tabElement" href="#skos-endpoint-tab1" style="text-decoration:none;padding: 10px 15px;"><span>1.</span> Constraint and SPARQL Endpoint</a></li>
		<li id="skos-endpoint-validation-tab"><a class="tabElement" href="#skos-endpoint-tab2" style="text-decoration:none;padding: 10px 15px;"><span>2.</span> Validation</a></li>
	</ul>
	
	<div id="skos-endpoint-tab1" style="padding:0 !important; border:none; float:left; width:100%">
		<#include "skos-endpoint-tab1.ftl" />
	</div>
	<div id="skos-endpoint-tab2" style="padding:0 !important;border:none;float:left; width:100%">
		<#include "skos-endpoint-tab2.ftl" />
	</div>
</div>
...
<script>
	$jQ( function() // begin document ready
	{
		...
		$jQ( "#skosEndpointTabContainer" ).tabs({ 
			active: 0, 
			activate : function(event, ui){
				if(ui.newPanel.selector == "#skos-endpoint-tab2")
					<#-- validate inputs -->
					ajaxskosendpointValidation( "#skos-validation-result" );
			}
     	});
     	
		<#-- submit button validation pressed -->
		$jQ( "#button_skos-endpoint-validation" ).click( function()
		{
			<#-- validate inputs -->
			ajaxskosendpointValidation( "#skos-validation-result" );
		} );
	...
	function getskosInputs( containerSelector ){
		<#-- clear -->
	...
	-->
	function ajaxskosendpointValidation( containerResultSelector ){	
		<#-- disable validate button -->
		<!-- $jQ('#button_skos-endpoint-validation').attr("disabled",true).addClass("buttonSubmitDissabled"); -->
		<#-- Add loading icon -->
		...
		$jQ.ajax( {
		    type: "post",
			url: "<@spring.url '/skos/endpoint/validation' />",
			data: {
