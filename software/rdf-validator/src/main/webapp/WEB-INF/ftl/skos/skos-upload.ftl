...
<ul style="height: 40px;">	
	<li><a class="tabElement" href="#skos-upload-tab1" style="text-decoration:none;padding: 10px 15px;"><span>1.</span> Constraints and Data</a></li>
	<li id="skos-upload-validation-tab"><a class="tabElement" href="#skos-upload-tab2" style="text-decoration:none;padding: 10px 15px;"><span>2.</span> Validation</a></li>
</ul>
...
<script>
	$jQ( function() // begin document ready
	{
		...
		$jQ( "#button_skos-upload-validation" ).click( function()
		{
			<#-- validate inputs -->
			ajaxSkosUploadValidation( "#skos-validation-result" );
		} );
		
		<#-- on validation tab click collect input -->
		$jQ( "#skos-upload-validation-tab" ).click( function(){
			getSkosInputs( "#skos-validation-inputs div" );
		} );
	...
	-->
	function ajaxSkosUploadValidation( containerResultSelector ){	
		<#-- disable validate button -->
		<!-- $jQ('#button_skos-upload-validation').attr("disabled",true).addClass("buttonSubmitDissabled"); -->
		<#-- Add loading icon -->
		...
		$jQ.ajax( {
			type: "post",
			url: "<@spring.url '/skos/upload/validation' />"
		    })
	    	.done( function( html ) {
