 <@layout.global>
 	...
 	<@content.left>
			<div id="col1_content" class="clearfix">
				...
				<div id="phddTabContainer" class="tabContainer">
					<ul>
						<li>
							<a class="tabElement" href="#study_tab" style="text-decoration:none;">PHDD</a>
						</li>
					</ul>
...
<script>
	$jQ( function() // begin document ready
	{
		...
		$jQ( "#phddTabContainer" ).tabs({ active: 0 });
		$jQ( "#phddTabContainer" ).css( "margin-top", "66px");
		$jQ( "#phddTabContainer" ).css( "margin-left", "5px");
		$jQ( "#phddTabContainer" ).css( "margin-right", "5px");
		...
		$jQ(".graphButton").click( function(){
			...
			if( selectedNav != currentNavButton ){
				...
				$jQ.ajax( {
					type: "get",
					url:  "<@spring.url '/phdd/' />" + selectedNav
				    })
			    	.done( function( html ) {
