...
<!-- the tabs -->
<div id="phddEndpointTabContainer" class="tabContainer">
	<ul style="height: 40px;">	
		<li><a class="tabElement" href="#phdd-endpoint-tab1" style="text-decoration:none;padding: 10px 15px;"><span>1.</span> Constraint and SPARQL Endpoint</a></li>
		<li id="phdd-endpoint-validation-tab"><a class="tabElement" href="#phdd-endpoint-tab2" style="text-decoration:none;padding: 10px 15px;"><span>2.</span> Validation</a></li>
	</ul>
	
	<div id="phdd-endpoint-tab1" style="padding:0 !important; border:none; float:left; width:100%">
		<#include "phdd-endpoint-tab1.ftl" />
	</div>
	<div id="phdd-endpoint-tab2" style="padding:0 !important;border:none;float:left; width:100%">
		<#include "phdd-endpoint-tab2.ftl" />
	</div>
</div>
...
<script>
	$jQ( function() // begin document ready
	{
		...
		$jQ( "#phddEndpointTabContainer" ).tabs({ 
			active: 0, 
			activate : function(event, ui){
				if(ui.newPanel.selector == "#phdd-endpoint-tab2")
					<#-- validate inputs -->
					ajaxphddendpointValidation( "#phdd-validation-result" );
			}
     	});
     	
		<#-- submit button validation pressed -->
		$jQ( "#button_phdd-endpoint-validation" ).click( function()
		{
			<#-- validate inputs -->
			ajaxphddendpointValidation( "#phdd-validation-result" );
		} );
	...
	-->
	function ajaxphddendpointValidation( containerResultSelector ){	
		<#-- disable validate button -->
		<!-- $jQ('#button_phdd-endpoint-validation').attr("disabled",true).addClass("buttonSubmitDissabled"); -->
		<#-- Add loading icon -->
		...
		$jQ.ajax( {
		    type: "post",
			url: "<@spring.url '/phdd/endpoint/validation' />",
			data: {
