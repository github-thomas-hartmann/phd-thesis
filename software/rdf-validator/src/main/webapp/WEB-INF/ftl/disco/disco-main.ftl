 <@layout.global>
 	...
 	<@content.left>
			<div id="col1_content" class="clearfix">
				...
				<div id="discoTabContainer" class="tabContainer">
					<ul>
						<li>
							<a class="tabElement" href="#study_tab" style="text-decoration:none;">disco</a>
						</li>
					</ul>
...
<script>
	$jQ( function() // begin document ready
	{
		...
		$jQ( "#discoTabContainer" ).tabs({ active: 0 });
		$jQ( "#discoTabContainer" ).css( "margin-top", "66px");
		$jQ( "#discoTabContainer" ).css( "margin-left", "5px");
		$jQ( "#discoTabContainer" ).css( "margin-right", "5px");
		...
		$jQ(".graphButton").click( function(){
			...
			if( selectedNav != currentNavButton ){
				...
				$jQ.ajax( {
					type: "get",
					url:  "<@spring.url '/disco/' />" + selectedNav
				    })
			    	.done( function( html ) {
