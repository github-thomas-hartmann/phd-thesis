...
<!-- the tabs -->
<div id="discoEndpointTabContainer" class="tabContainer">
	<ul style="height: 40px;">	
		<li><a class="tabElement" href="#disco-endpoint-tab1" style="text-decoration:none;padding: 10px 15px;"><span>1.</span> Constraint and SPARQL Endpoint</a></li>
		<li id="disco-endpoint-validation-tab"><a class="tabElement" href="#disco-endpoint-tab2" style="text-decoration:none;padding: 10px 15px;"><span>2.</span> Validation</a></li>
	</ul>
	
	<div id="disco-endpoint-tab1" style="padding:0 !important; border:none; float:left; width:100%">
		<#include "disco-endpoint-tab1.ftl" />
	</div>
	<div id="disco-endpoint-tab2" style="padding:0 !important;border:none;float:left; width:100%">
		<#include "disco-endpoint-tab2.ftl" />
	</div>
</div>
...
<script>
	$jQ( function() // begin document ready
	{
		...
		$jQ( "#discoEndpointTabContainer" ).tabs({ 
			active: 0, 
			activate : function(event, ui){
				if(ui.newPanel.selector == "#disco-endpoint-tab2")
					<#-- validate inputs -->
					ajaxdiscoendpointValidation( "#disco-validation-result" );
			}
     	});
     	
		<#-- submit button validation pressed -->
		$jQ( "#button_disco-endpoint-validation" ).click( function()
		{
			<#-- validate inputs -->
			ajaxdiscoendpointValidation( "#disco-validation-result" );
		} );
	...
	function getdiscoInputs( containerSelector ){
		<#-- clear -->
	...
	-->
	function ajaxdiscoendpointValidation( containerResultSelector ){	
		<#-- disable validate button -->
		<!-- $jQ('#button_disco-endpoint-validation').attr("disabled",true).addClass("buttonSubmitDissabled"); -->
		<#-- Add loading icon -->
		...
		$jQ.ajax( {
		    type: "post",
			url: "<@spring.url '/disco/endpoint/validation' />",
			data: {
