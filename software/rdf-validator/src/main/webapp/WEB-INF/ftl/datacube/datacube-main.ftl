 <@layout.global>
 	...
 	<@content.left>
			<div id="col1_content" class="clearfix">
				...
				<div id="datacubeTabContainer" class="tabContainer">
					<ul>
						<li>
							<a class="tabElement" href="#study_tab" style="text-decoration:none;">Data Cube</a>
						</li>
					</ul>
...
<script>
	$jQ( function() // begin document ready
	{
		...
		$jQ( "#datacubeTabContainer" ).tabs({ active: 0 });
		$jQ( "#datacubeTabContainer" ).css( "margin-top", "66px");
		$jQ( "#datacubeTabContainer" ).css( "margin-left", "5px");
		$jQ( "#datacubeTabContainer" ).css( "margin-right", "5px");
		...
		$jQ(".graphButton").click( function(){
			...
			if( selectedNav != currentNavButton ){
				...
				$jQ.ajax( {
					type: "get",
					url:  "<@spring.url '/datacube/' />" + selectedNav
				    })
			    	.done( function( html ) {
