...
<ul style="height: 40px;">	
	<li><a class="tabElement" href="#dsp2-upload-tab1" style="text-decoration:none;padding: 10px 15px;"><span>1.</span> Constraints and Data</a></li>
	<li id="dsp2-upload-validation-tab"><a class="tabElement" href="#dsp2-upload-tab2" style="text-decoration:none;padding: 10px 15px;"><span>2.</span> Validation</a></li>
</ul>

<div id="dsp2-upload-tab1" style="padding:0 !important; border:none; float:left; width:100%">
	<#include "dsp2-upload-tab1.ftl" />
</div>
<div id="dsp2-upload-tab2" style="padding:0 !important;border:none;float:left; width:100%">
	<#include "dsp2-upload-tab2.ftl" />
</div>
...
<script>
	$jQ( function() // begin document ready
	{
		...
		$jQ( "#button_dsp2-upload-validation" ).click( function()
		{
			<#-- validate inputs -->
			ajaxDspUploadValidation( "#dsp2-validation-result" );
		} );
		
		<#-- on validation tab click collect input -->
		$jQ( "#dsp2-upload-validation-tab" ).click( function(){
			getDSPInputs( "#dsp2-validation-inputs div" );
		} );
	...
	function ajaxDspUploadValidation( containerResultSelector ){	
		...
		<!-- $jQ('#button_dsp2-upload-validation').attr("disabled",true).addClass("buttonSubmitDissabled"); -->
		<#-- Add loading icon -->
		...
		$jQ.ajax( {
			type: "post",
			url: "<@spring.url '/dsp2/upload/validation' />"
		    })
	    	.done( function( html ) {
