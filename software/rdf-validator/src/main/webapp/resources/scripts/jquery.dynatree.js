...
DynaTreeNode.prototype = {
	initialize: function(parent, tree, data) {
		...
		data.nodeNumber = tree._nodeNumber++;
	...
	search : function(pattern,minTriggerLength){
	    if(nodeKeyList.length > 0){
	        var i=0;
	        this.visit(function(node){
	            if(node.data.key === nodeKeyList[i]){
	            	node.data.addClass = "";
		            node.render();
	            	if(i<nodeKeyList.length-1)
	            		i++;
	            }
	        });
	        nodeKeyList =[];
	    } 
	    if (pattern.length >= minTriggerLength) {
	        for (var i = 0; i < this.childList.length; i++){
	            this.childList[i]._searchNode(pattern);
	        }
	        return nodeKeyList;
	    } 
	},
	_searchNode : function(pattern){
	    if (this.childList){
	    	// parent node
	        for(var i = 0; i < this.childList.length; i++){
	            this.childList[i]._searchNode(pattern);
	        ...
	        if(this._isRightWithPattern(pattern)){
	        	//nodeKeyList.push(this.data.key);
	        	//this.data.addClass = "samePatternNode";
	            //this.render();
	        }
	    } else {
	        // leaf
...
//				self.tree.logDebug("appendAjax().success");
	...
	move: function(targetNode, mode) {
		...
		if( this.parent.childList.length == 1 ) {
			...
			if( pos < 0 ){
				// throw "Internal error"; //for enable redo
			}
			if(pos>-1) this.parent.childList.splice(pos, 1);
		}
...
//  disabled: false,
	...
	// ------------------------------------------------------------------------
	nodeNumber: null, //added atribute for tree searching
	lastentry: undefined
};
