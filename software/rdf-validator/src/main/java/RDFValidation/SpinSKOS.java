...
//	public PrintWriter writerInferredTriples = null;
	...
	// zip files
	private List<String> zipFiles = null;
	 ...
	 * 
	 */
	public SpinSKOS( String... spinMappings )
	{
		// initialization
		init( spinMappings );
	}
...
//		debugging = debugging + "</li>";
		...
		// get RDF graphs ( from zip files )
		if ( zipFiles != null )
		{
			for ( String zipFile : zipFiles ) 
			{
				graph.add( getRDFGraphFromZip( "/thesauri/" + zipFile, "TTL" ) );
			}
		}
...
//		System.out.println(this.getClass().getClassLoader().getResourceAsStream( "/" + rdfGraph_RelativePathAndFileName ));
	...
	public Model getRDFGraphFromZip( String relativePathAndFileName, String rdfGraph_ConcreteSyntax )
	{
		...
		{ 
			...
			// create SPIN exception
			spinException = new SPINException();
			spinException.setSource( relativePathAndFileName );
			spinException.setMessage( e.getMessage().replace( "<", "&lt;" ).replace( ">", "&gt;" ) );
			
			System.out.println( "-----" ); 
			System.out.println( "source: " + relativePathAndFileName ); 
			System.out.println( "message: " + e.getMessage() ); 
			System.out.println( "-----" ); 
		}
